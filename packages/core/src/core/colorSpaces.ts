...
function srgbToLinear(channel: number): number {
  ...
  return c <= SRGB_TO_LINEAR_THRESHOLD
    ...
    : ((c + SRGB_TO_LINEAR_OFFSET) / 1.055) ** SRGB_TO_LINEAR_GAMMA
}
...
function linearToSrgb(channel: number): number {
  const c = channel <= LINEAR_TO_SRGB_THRESHOLD
    ...
    : LINEAR_TO_SRGB_A * channel ** LINEAR_TO_SRGB_GAMMA - SRGB_TO_LINEAR_OFFSET
...
export function deltaE2000(rgb1: RGB, rgb2: RGB): number {
  ...
  // Calculate RT
  const deltaTheta = 30 * Math.exp(-(((hPrimeAvg - 275) / 25) ** 2))
  const rC = 2 * Math.sqrt(cPrimeAvg ** 7 / (cPrimeAvg ** 7 + 25 ** 7))
