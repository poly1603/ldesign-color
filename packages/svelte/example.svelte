<script lang="ts">
  /**
   * @ldesign/color-svelte 使用示例
   * 
   * 这个示例展示了如何在 Svelte 应用中使用颜色主题管理
   */
  ...
  // 应用预设主题
  function handlePresetTheme(presetName: string) {
  ...
  function handleModeChange(event: CustomEvent) {
    console.log('模式已更改:', event.detail)
  }
</script>
    ...
    <section class="info-panel">
      ...
      {:else}
        <div class="theme-info">
          ...
          <div class="info-item">
            <label>主题色:</label>
            <span class="color-preview" style="background-color: {$primaryColor}">
              {$primaryColor}
            </span>
          </div>
      ...
      <div class="preset-grid">
        {#each $presets as preset}
          <button
            ...
            on:click={() => handlePresetTheme(preset.name)}
