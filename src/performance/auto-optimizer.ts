...
export class ColorAutoOptimizer {
  ...
  applyProfile(profileName: string): void {
    ...
    if ((globalColorCache as any).maxSize !== profile.config.cacheSize) {
      ...
      changes.push(`基础缓存: ${profile.config.cacheSize}项`)
    }

    if ((advancedCache as any).maxSize !== profile.config.advancedCacheSize) {
      (advancedCache as any).maxSize = profile.config.advancedCacheSize
      changes.push(`高级缓存: ${profile.config.advancedCacheSize}项`)
    }

    // 调整缓存策略
    advancedCache.setStrategy(profile.config.cacheStrategy)
