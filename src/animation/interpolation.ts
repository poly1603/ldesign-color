...
import { oklabToRGB, oklchToOKLAB, oklchToRGB, rgbToOKLAB, rgbToOKLCH } from '../core/advancedColorSpaces';
import { hslToRgb, rgbToHsl, rgbToHsv, hsvToRgb } from '../core/conversions';
...
const EASING_FUNCTIONS: Record<string, (t: number) => number> = {
  ...
  'ease-in-expo': (t: number) => t === 0 ? 0 : Math.pow(2, 10 * t - 10),
...
export class ColorInterpolator {
  ...
  private interpolateLAB(t: number): Color {
    const { rgbToLAB, labToRGB } = require('../core/advancedColorSpaces');
