 ...
 * Minified: No                    *
 * ***********************************
 */function i(r,o){return`--color-${r}-${o}`}function g(r,o){return Object.entries(o).map(([a,c])=>`  ${i(r,a)}: ${c};`).join(`
`)}function u(r){const o=[];return Object.entries(r).forEach(([a,c])=>{o.push(`  /* ${a.charAt(0).toUpperCase()+a.slice(1)} */`),o.push(g(a,c))}),o.join(`
`)}function l(r){const o=u(r.light),a=u(r.dark);return`/* Light Mode (Default) */
:root {
${o}
}
...
${u(r.dark)}
  ...
  --color-background-secondary: var(--color-bg-container-secondary);
  --color-background-tertiary: var(--color-bg-container-tertiary);
}`}function s(r,o=!0){const a=o?t(r):l(r);let c=document.getElementById("ldesign-color-theme");c||(c=document.createElement("style"),c.id="ldesign-color-theme",document.head.appendChild(c)),c.textContent=a}function e(r){document.documentElement.setAttribute("data-theme-mode",r)}function n(){return document.documentElement.getAttribute("data-theme-mode")==="dark"?"dark":"light"}function d(){const r=n()==="light"?"dark":"light";return e(r),r}function v(){const r=localStorage.getItem("theme-mode");if(r==="dark"||r==="light")e(r);else{const o=window.matchMedia("(prefers-color-scheme: dark)").matches;e(o?"dark":"light")}window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",o=>{localStorage.getItem("theme-mode")||e(o.matches?"dark":"light")})}function p(r){localStorage.setItem("theme-mode",r),e(r)}exports.generateSemanticCssVariables=t,exports.generateThemedCssVariables=l,exports.getThemeMode=n,exports.initThemeMode=v,exports.injectThemedCssVariables=s,exports.saveThemeMode=p,exports.setThemeMode=e,exports.toggleThemeMode=d;/*! End of @ldesign/color | Powered by @ldesign/builder */
//# sourceMappingURL=cssVariables.cjs.map
